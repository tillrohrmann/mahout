...
object FlinkOpAtB {
  ...
  class FlatMapper[K](val blockCount: Int, blockHeight: Int, ncol: Int) extends FlatMapFunction[Tuple2[Tuple2[K, Vector], Tuple2[K, Vector]],
    Tuple2[Int, Matrix]] with ResultTypeQueryable[Tuple2[Int, Matrix]]{
    ...
    override def getProducedType: TypeInformation[Tuple2[Int, Matrix]] = {
      createTypeInformation[Tuple2[Int, Matrix]]
    }
  }

  def notZippable[K: ClassTag: TypeInformation](op: OpAtB[K], At: FlinkDrm[K], B: FlinkDrm[K]): FlinkDrm[Int] = {
    // TODO: to help Flink's type inference
    // only Int is supported now 
    val rowsAt = At.deblockify.ds.map(new Mapper[K])
    val rowsB = B.deblockify.ds.map(new Mapper[K])
    ...
    val preProduct = joined.flatMap(new FlatMapper[K](blockCount, blockHeight, ncol))
